...
class Solution
{
    ...
    private static void Help(Node root,int sum,ArrayList<ArrayList<Integer>> ans,List<Integer> a){
        if(root==null)return;
        a.add(root.data);
        sum-=root.data;
        if(sum==0){
        ...
        Help(root.left,sum,ans,a);
        Help(root.right,sum,ans,a);
        a.remove(a.size()-1);
    ...
    public static ArrayList<ArrayList<Integer>> printPaths(Node root, int sum)
    {
        ...
        Help(root,sum,ans,new LinkedList<>());
        return ans;
